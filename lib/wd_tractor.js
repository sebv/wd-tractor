 ...
 * Angular.
 */
WdTractor.prototype.get = function(destination, done) {
...
(function() {
  ...
  var orig = WdTractor.prototype.element;
  WdTractor.prototype.element = function() {
    ...
    if(locatorType === 'ng binding'){
      this.execute(clientSideScripts.findBinding, [locatorValue], function(err, el) {
  ...
  orig = WdTractor.prototype.elements;
  WdTractor.prototype.elements = function() {
    ...
    if(locatorType === 'ng binding'){
      this.execute(clientSideScripts.findBindings, [locatorValue], function(err, els) {
        if(err) { return cb(err);}
        cb(null, _(els).map(function(el) {return el.value;}));
      });
    } else {
      orig.apply(this, args);
    }
  ...
  WdTractor.prototype._buildBySuffixMethods('ng binding', WdTractor.prototype);
