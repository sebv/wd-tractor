  ...
  describe('synchronizing with Angular', function() {
    ...
    it('should wait for slow RPCs', function(done) {
      async.series([
        ...
        async.series([
          function(done) { fetchButton.click(done); },
      ...
      it('should synchronize with a slow action', function(done) {
        ...
        async.waterfall([
          ...
          function(_addOneButton, done) {
            ...
            browser.clickElement(addOneButton, done);
          },
          function(done) {browser.elementByNgRepeater('foo in foos', 1, '{{foo.b}}', done);},
          function(topNumberElem , done) {
            elemTextShouldEqual(browser, topNumberElem, '14930352', done);
          },
          function(done) {browser.clickElement(addOneButton, done);},
          function(done) {browser.elementByNgRepeater('foo in foos', 1, '{{foo.b}}', done);},
          function(topNumberElem , done) {
            elemTextShouldEqual(browser, topNumberElem, '24157817', done);
          }
        ], done);
      });
    });
